...
export function parsePayloadProfile(value) {
  ...
  for (const entry of String(value || '').split(',')) {
    const [mimeType, payloadType] = entry.trim().split('=');
    const parsed = Number(payloadType);
    if (mimeType && Number.isInteger(parsed)) profile.set(mimeType.toLowerCase(), parsed);
  }
  return profile;
}
...
function codecPayloads(section, mimeType) {
  ...
  return [...section.matchAll(/^a=rtpmap:(\d+)\s+([^/\s]+)\/\d+(?:\/\d+)?\s*$/gim)]
    .filter((match) => match[2].toLowerCase() === normalized.split('/')[1])
...
function remapPayloadType(section, mimeType, targetPayloadType) {
  ...
  if (assigned === codecName) return section;
...
export function waitForIceGathering(peer, timeoutMs = 12_000) {
...
function protocolFailure(protocol, response, body) {
  let detail = '';
  try {
...
export async function publishWhip({
  ...
  try {
    ...
    if (response.status !== 201) throw protocolFailure('WHIP publish', response, answer);
...
export async function consumeWhep({
  ...
  try {
    ...
    const answer = await response.text();
    if (response.status !== 201) throw protocolFailure('WHEP playback', response, answer);
    const location = response.headers.get('location');
...
export function resolveScreenPresentation({ localScreenActive, remoteScreenActive }) {
  if (remoteScreenActive) return { primary: 'remote-screen', fit: 'contain', swapLocked: true };
  if (localScreenActive) return { primary: 'local-screen', fit: 'contain', swapLocked: true };
  return { primary: null, fit: 'cover', swapLocked: false };
}
