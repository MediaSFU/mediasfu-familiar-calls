...
enum FamiliarAPIError: LocalizedError {
    case invalidURL, invalidResponse, server(String), unavailable
...
final class FamiliarCallsAPI {
    ...
    init(origin: URL? = URL(string: ProcessInfo.processInfo.environment["CALL_API_ORIGIN"] ?? "http://127.0.0.1:8790")!) {
        self.origin = origin ?? URL(string: "http://127.0.0.1:8790")!
    }

    func sessions(for userId: String) async throws -> [FamiliarSession] {
        let payload: SessionListResponse = try await request(path: "/api/users/\(userId.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? userId)/sessions")
        return payload.data.sorted { $0.updatedAt > $1.updatedAt }
    }

    func create(hostUserId: String, targetUserId: String, displayName: String, callType: String) async throws -> (FamiliarSession, [String: Any]) {
        let body: [String: Any] = ["hostUserId": hostUserId, "targetUserId": targetUserId, "displayName": displayName, "callType": callType, "duration": 30, "capacity": 2, "eventType": "conference"]
        let response = try await requestRaw(path: "/api/rooms/create", method: "POST", body: body)
        return (try decode(FamiliarSession.self, from: response["session"]), response["data"] as? [String: Any] ?? [:])
    }

    func join(sessionId: String, userId: String, displayName: String) async throws -> (FamiliarSession, [String: Any]) {
        let body: [String: Any] = ["sessionId": sessionId, "userId": userId, "displayName": displayName]
    ...
    func end(sessionId: String, userId: String, reason: String = "user_ended") async throws {
        _ = try await requestRaw(path: "/api/sessions/\(sessionId)/end", method: "POST", body: ["userId": userId, "reason": reason])
    }
    ...
    private func requestRaw(path: String, method: String = "GET", body: [String: Any]? = nil) async throws -> [String: Any] {
        guard let url = URL(string: path, relativeTo: origin)?.absoluteURL else { throw FamiliarAPIError.invalidURL }
        ...
        do {
            ...
            if !(200..<300).contains(http.statusCode) || (object["success"] as? Bool) == false {
                throw FamiliarAPIError.server(object["error"] as? String ?? "The call service could not complete the request.")
            }
            return object
        } catch let error as FamiliarAPIError { throw error }
        catch { throw FamiliarAPIError.unavailable }
    }
}
